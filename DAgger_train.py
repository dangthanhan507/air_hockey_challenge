...
class CustomEnv(gym.Env):
  ...
  def __init__(self, mdp_name):
    ...
    bc_obs_space.low[6:9] = np.array([-10,-10,-10])
    bc_obs_space.high[6:9] = np.array([10,10,10])

    bc_obs_space.low[9:12] = np.array([-10,-10,-10])
    bc_obs_space.high[9:12] = np.array([10,10,10])
...
# venv = SubprocVecEnv([CustomEnv('3dof-hit') for i in range(3)])
...
custom_policy = AirHockeyACPolicy(env.observation_space, env.action_space)
